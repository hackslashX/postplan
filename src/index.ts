...
export function createMcpServer(service: ProjectService, publicBaseUrl: string) {
  ...
  server.registerTool("get_ui_skill", { description: "Get detailed, practical UI guidance for one skill returned by list_ui_skills. Use the advice to design project HTML, CSS, and JavaScript.", inputSchema: { skill_id: z.string().describe("A skill id from list_ui_skills") } }, async ({ skill_id }) => { const skill = findUiSkill(skill_id); return skill ? text(skill) : fail(`Unknown UI skill: ${skill_id}. Call list_ui_skills first.`); });
  ...
  server.registerTool("read_file", { description: "Read a file. For text files, optionally select lines or search matching lines. Binary files return base64 content and cannot be line-selected.", inputSchema: { project_id: z.string(), key: z.string(), path: z.string(), start_line: z.number().int().positive().optional(), line_count: z.number().int().positive().max(10000).optional(), query: z.string().optional(), regex: z.boolean().optional(), context_lines: z.number().int().min(0).max(100).optional() } }, async ({ project_id, key, path: filePath, start_line, line_count, query, regex, context_lines }) => {
    try {
      ...
      if (query !== undefined) { const matcher = regex ? new RegExp(query, "i") : { test: (value: string) => value.toLowerCase().includes(query.toLowerCase()) }; const hits = selected.filter(item => matcher.test(item.content)).map(item => item.line); const padding = context_lines ?? 0; const wanted = new Set(hits.flatMap(hit => Array.from({ length: padding * 2 + 1 }, (_, i) => hit - padding + i)).filter(line => line >= 1 && line <= lines.length)); selected = selected.filter(item => wanted.has(item.line)); }
...
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Postplan — plans, in a browser</title><style>
...
<body><main class="wrap"><nav><div><span class="mark">✦</span>postplan</div><div class="tag">MCP-powered web hosting</div></nav><section class="hero"><h1>Plans belong<br>in a <span class="accent">browser.</span></h1><p>Postplan lets AI agents turn HTML plans, prototypes, and visual notes into a private link you can open on any device.</p></section><section class="flow"><div class="step"><span class="num">01</span><b>An agent creates</b>It gets an anonymous project, a write key, and a separate share key.</div><div class="step"><span class="num">02</span><b>It builds</b>HTML, CSS, JavaScript, images, and other assets are uploaded through MCP.</div><div class="step"><span class="num">03</span><b>You open</b>The agent returns a browser link. No file download. No local setup.</div></section><pre class="code"><span>POST</span> ${html(publicBaseUrl)}/mcp

Agent → create_project → put_file(index.html) → share view_url
You    → open the link, anywhere</pre><footer>Anonymous projects · Read-only sharing links · SQLite + persistent disk</footer></main></body></html>`);
  });
  app.get("/view", (req, res) => {
    ...
    res.set("Content-Security-Policy", "default-src 'none'; style-src 'unsafe-inline'; frame-src 'self'");
    res.type("html").send(`<!doctype html><meta name="viewport" content="width=device-width,initial-scale=1"><title>Postplan</title><style>html,body,iframe{margin:0;width:100%;height:100%;border:0}body{min-height:100vh}</style><iframe sandbox="allow-scripts allow-forms allow-modals allow-popups" src="${html(source)}" title="Hosted project"></iframe>`);
  });
  app.get(/^\/project\/([^/]+)\/([^/]+)\/(.+)$/, async (req, res) => {
    const [, id, key, filePath] = req.path.match(/^\/project\/([^/]+)\/([^/]+)\/(.+)$/) ?? [];
    if (!id || !key || !filePath) { res.sendStatus(404); return; }
    try { const file = await service.getFile(id, key, decodeURIComponent(filePath)); if (!file?.content) { res.sendStatus(404); return; } res.set({ "Content-Type": file.metadata.mimeType, "X-Content-Type-Options": "nosniff", "Cache-Control": "private, max-age=300" }).send(file.content); } catch { res.sendStatus(404); }
  });
  return app;
}
